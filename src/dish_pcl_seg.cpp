...
// Create conversions
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_dish (new pcl::PointCloud<pcl::PointXYZ> ());
  ...
  extract.filter (*cloud_dish);
  
  // Publish the inlier data (table)
  time_st = ros::Time::now ();
  cloud_dish->header.frame_id = "camera_rgb_optical_frame";
  cloud_dish->header.stamp = time_st.toNSec()/1e3;
  pcl_pub.publish (cloud_dish);
/*
  // Display Pointcloud
  pcl::visualization::CloudViewer viewer ("Simple Cloud Viewer");
  viewer.showCloud (cloud_dish);
...
{
  ros::init(argc, argv, "dish_pcl_seg");
/*
...
*/
  ...
  pcl_full_pub = nh.advertise<sensor_msgs::PointCloud2> ("/dish/full_roi_pcl", 1);
  pcl_pub = nh.advertise<sensor_msgs::PointCloud2> ("/dish/pcl_seg/pcl", 1);
  img_pub = nh.advertise<sensor_msgs::Image> ("/dish/dish_rect_color", 1);
  depth_pub = nh.advertise<sensor_msgs::Image> ("/dish/dish_rect_depth", 1);
