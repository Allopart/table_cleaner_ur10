...
// **************** Kmeans color clustering *********************
  ...
  // Convert Roi to binary
    ...
    cvtColor(roi_img,bwImage,CV_RGB2GRAY);
  ...
  black_roi.copyTo(final_contours(Rect(mask_coeff[0], mask_coeff[1],mask_coeff[2], mask_coeff[3])));
...
{
  ros::init(argc, argv, "glass_vis_seg");
...
*/
  ...
  ros::Subscriber mask_coeff = nh.subscribe("/glass/mask_coeff", 1, mask_callback);
...
//message_filters::Subscriber<sensor_msgs::Image> depth_sub(nh, "/glass/glass_rect_depth", 1);


  pcl_pub = nh.advertise<sensor_msgs::PointCloud2> ("/glass/vis_seg/pcl", 1);
