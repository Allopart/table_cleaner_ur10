...
// Create conversions
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_knife (new pcl::PointCloud<pcl::PointXYZ> ());
  ...
  extract.filter (*cloud_knife);
  

  // Publish the inlier data (table)
  time_st = ros::Time::now ();
  cloud_knife->header.frame_id = "camera_rgb_optical_frame";
  cloud_knife->header.stamp = time_st.toNSec()/1e3;
  pcl_pub.publish (cloud_knife);
/*
  // Display Pointcloud
  pcl::visualization::CloudViewer viewer ("Simple Cloud Viewer");
  viewer.showCloud (cloud_knife);
...
{
  ros::init(argc, argv, "knife_pcl_seg");
/*
...
*/
  ...
  ros::NodeHandle nh;
  message_filters::Subscriber<sensor_msgs::Image> mask_sub(nh, "/cropped/knife_mask", 1);
  ...
  pcl_full_pub = nh.advertise<sensor_msgs::PointCloud2> ("/knife/full_roi_pcl", 1);
  pcl_pub = nh.advertise<sensor_msgs::PointCloud2> ("/knife/pcl_seg/pcl", 1);
  img_pub = nh.advertise<sensor_msgs::Image> ("/knife/knife_rect_color", 1);
  depth_pub = nh.advertise<sensor_msgs::Image> ("/knife/knife_rect_depth", 1);
