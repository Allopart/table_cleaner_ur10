...
void callback(const sensor_msgs::PointCloud2ConstPtr& rgbd, const sensor_msgs::PointCloud2ConstPtr& pcl)
{
  ...
  pcl_conversions::toPCL(*rgbd, *cloud);
  ...
  pcl::fromPCLPointCloud2(cloud_filtered, cloud_rgbd);
...
{
  ros::init(argc, argv, "dish_fusion");
  
  ros::NodeHandle nh;
  message_filters::Subscriber<sensor_msgs::PointCloud2> pcl_sub(nh, "/dish/pcl_seg/pcl", 1);
  message_filters::Subscriber<sensor_msgs::PointCloud2> rgbd_sub(nh, "/dish/vis_seg/pcl", 1);
  final_pub = nh.advertise<sensor_msgs::PointCloud2> ("/dish/final/pcl", 1);
