...
// Create conversions
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_glass (new pcl::PointCloud<pcl::PointXYZ> ());
  ...
  extract.filter (*cloud_glass);
  
  // Publish the inlier data (table)
  time_st = ros::Time::now ();
  cloud_glass->header.frame_id = "camera_rgb_optical_frame";
  cloud_glass->header.stamp = time_st.toNSec()/1e3;
  pcl_pub.publish (cloud_glass);
/*
  // Display Pointcloud
  pcl::visualization::CloudViewer viewer ("Simple Cloud Viewer");
  viewer.showCloud (cloud_glass);
...
{
  ros::init(argc, argv, "glass_pcl_seg");
/*
...
*/
  ...
  ros::NodeHandle nh;
  message_filters::Subscriber<sensor_msgs::Image> mask_sub(nh, "/cropped/glass_mask", 1);
  ...
  pcl_full_pub = nh.advertise<sensor_msgs::PointCloud2> ("/glass/full_roi_pcl", 1);
  pcl_pub = nh.advertise<sensor_msgs::PointCloud2> ("/glass/pcl_seg/pcl", 1);
  img_pub = nh.advertise<sensor_msgs::Image> ("/glass/glass_rect_color", 1);
  depth_pub = nh.advertise<sensor_msgs::Image> ("/glass/glass_rect_depth", 1);
