...
// Create conversions
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_fork (new pcl::PointCloud<pcl::PointXYZ> ());
  ...
  extract.filter (*cloud_fork);
  

  // Publish the inlier data (table)
  time_st = ros::Time::now ();
  cloud_fork->header.frame_id = "camera_rgb_optical_frame";
  cloud_fork->header.stamp = time_st.toNSec()/1e3;
  pcl_pub.publish (cloud_fork);
/*
  // Display Pointcloud
  pcl::visualization::CloudViewer viewer ("Simple Cloud Viewer");
  viewer.showCloud (cloud_fork);
...
{
  ros::init(argc, argv, "fork_pcl_seg");
/*
...
*/
  ...
  pcl_full_pub = nh.advertise<sensor_msgs::PointCloud2> ("/fork/full_roi_pcl", 1);
  pcl_pub = nh.advertise<sensor_msgs::PointCloud2> ("/fork/pcl_seg/pcl", 1);
  img_pub = nh.advertise<sensor_msgs::Image> ("/fork/fork_rect_color", 1);
  depth_pub = nh.advertise<sensor_msgs::Image> ("/fork/fork_rect_depth", 1);
